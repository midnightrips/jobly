...
describe("POST /users/:username/jobs/:jobId", function () {
  test("should allow user to apply to job", async function () {
    ...
    expect(response.body).toEqual({ applied: job.id });
  }); // fails because the id is constantly changing

  test("should return unauthorized if user is not logged in", async function () {
    const response = await request(app)
